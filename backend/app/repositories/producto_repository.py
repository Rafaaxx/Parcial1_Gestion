...
class ProductoRepository(BaseRepository[Producto]):
    ...
    async def list_with_filters(
        ...
        count_query = (
            select(func.count()).select_from(Producto).where(Producto.deleted_at.is_(None))
        )
        ...
        if allergen_ids:
            ...
            allergen_subquery = select(ProductoIngrediente.producto_id).where(
                ProductoIngrediente.ingrediente_id.in_(allergen_ids)
            )
    ...
    async def find_by_nombre(self, nombre: str) -> Optional[Producto]:
        ...
        query = select(self.model).where(
            self.model.nombre == nombre, self.model.deleted_at.is_(None)
        )
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def update_stock_atomic(self, producto_id: int, new_stock: int) -> Optional[Producto]:
        """
    ...
    async def toggle_disponibilidad(self, producto_id: int, disponible: bool) -> Optional[Producto]:
        """
        ...
        self.logger.debug(f"Toggled disponibilidad for producto_id={producto_id} to {disponible}")
        return producto
    ...
    async def add_categoria(
        ...
        self.logger.debug(f"Added categoria_id={categoria_id} to producto_id={producto_id}")
        return association

    async def remove_categoria(self, producto_id: int, categoria_id: int) -> bool:
        """
        ...
        self.logger.debug(f"Removed categoria_id={categoria_id} from producto_id={producto_id}")
        return True

    async def set_principal_categoria(self, producto_id: int, categoria_id: int) -> bool:
        """
    ...
    async def add_ingrediente(
        self, producto_id: int, ingrediente_id: int, es_removible: bool = True
    ...
    async def remove_ingrediente(self, producto_id: int, ingrediente_id: int) -> bool:
        """
        ...
        self.logger.debug(f"Removed ingrediente_id={ingrediente_id} from producto_id={producto_id}")
        return True
