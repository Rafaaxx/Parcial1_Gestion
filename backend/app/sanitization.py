...
def sanitize_dict(data: Dict[str, Any], keys_to_sanitize: List[str] = None) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            result[key] = [sanitize_dict(item, keys_to_sanitize) if isinstance(item, dict) else 
                          (escape_html(item) if isinstance(item, str) and (keys_to_sanitize is None or key in keys_to_sanitize) else item) 
                          for item in value]
        else:
            result[key] = value
