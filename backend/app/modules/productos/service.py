...
class ProductoService:
    ...
    async def list_productos(
        ...
        items = [
            self._build_producto_list_item(p, include_stock=include_stock)
            for p in productos
        ]
        ...
        if "nombre" in update_data and update_data["nombre"]:
            ...
            if new_name != producto.nombre:
                ...
                if existing and existing.id != producto_id:
                    raise ValueError(
                        f"Ya existe un producto con el nombre '{new_name}'"
                    )
    ...
    async def update_stock(
        ...
        producto = await self.repository.update_stock_atomic(
            producto_id,
            data.stock_cantidad
        )
        if not producto:
        ...
        logger.info(
            f"Updated stock for producto id={producto_id} to {data.stock_cantidad}"
        )
    ...
    async def toggle_disponibilidad(
        ...
        """
        producto = await self.repository.toggle_disponibilidad(
            producto_id,
            data.disponible
        )
        if not producto:
        ...
        logger.info(
            f"Toggled disponibilidad for producto id={producto_id} to {data.disponible}"
        )
        return self._build_producto_detail(producto, include_stock=True)
    ...
    async def remove_ingrediente(
        ...
        Raises:
            ValueError: If association not found
        """
        deleted = await self.repository.remove_ingrediente(
            producto_id,
            ingrediente_id
        )
        if not deleted:
