...
class TestRegister:
    ...
    async def test_register_success(
        ...
        mock_refresh_service.create_token.assert_called_once_with(
            usuario_id=1, uow=mock_uow
        )
...
class TestLogin:
    ...
    async def test_login_success(
        ...
        result = await auth_service.login(
            request=request, uow=mock_uow, auth_repo=mock_auth_repo
        )
    ...
    async def test_login_email_not_found(
        self, auth_service, mock_uow, mock_auth_repo
    ):
    ...
    async def test_login_inactive_account(
        ...
        user = make_user(
            id=1, email="test@example.com", password="Test1234!", activo=False
        )
        mock_auth_repo.find_by_email.return_value = user
...
class TestLogout:
    ...
    async def test_logout_success(
        self, auth_service, mock_uow, mock_refresh_service
    ):
        ...
        await auth_service.logout(
            refresh_token="valid-uuid", current_user=user, uow=mock_uow
        )
    ...
    async def test_logout_token_not_found(
        ...
        mock_refresh_service.revoke_token.side_effect = UnauthorizedError(
            "Token inválido"
        )
        ...
        with pytest.raises(UnauthorizedError) as exc:
            await auth_service.logout(
                refresh_token="invalid-uuid", current_user=user, uow=mock_uow
            )
